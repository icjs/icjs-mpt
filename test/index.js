...
tape('simple save and retrive', function (tester) {
  ...
  it('should update a value', function (t) {
    trie.put('test', Buffer.from('two'), function () {
      trie.get('test', function (err, value) {
...
tape('testing deletions cases', function (tester) {
  ...
  it('should delete from a extention->branch-extention', function (t) {
    trie.put(Buffer.from([11, 11, 11]), 'first', function () {
      // create the top branch
      trie.put(Buffer.from([12, 22, 22]), 'create the first branch', function () {
        // crete the middle branch
        trie.put(Buffer.from([12, 33, 33]), 'create the middle branch', function () {
          trie.put(Buffer.from([12, 33, 44]), 'create the last branch', function () {
  ...
  it('should delete from a extention->branch-branch', function (t) {
    trie.put(Buffer.from([11, 11, 11]), 'first', function () {
      ...
      trie.put(Buffer.from([12, 22, 22]), 'create the first branch', function () {
        // crete the middle branch
        trie.put(Buffer.from([12, 33, 33]), 'create the middle branch', function () {
          trie.put(Buffer.from([12, 34, 44]), 'create the last branch', function () {
            // delete the middle branch
            trie.del(Buffer.from([11, 11, 11]), function () {
              trie.get(Buffer.from([11, 11, 11]), function (err, val) {
                t.equal(null, val)
...
tape('it should create the genesis state root from irchain', function (tester) {
  var it = tester.test
  var trie4 = new Trie()
  ...
  startAmount.fill(0)
  startAmount[0] = 1
  var account = [startAmount, 0, stateRoot, ircUtil.sha3()]
